...
Every unresolved item on this site, in one table: **179 open items** across **42 pages**, of which **54 block the public release**.
...
| [safety → Rules](../before-you-start/safety.md#rules) | Power-on and power-off order (computer, USB-CAN adapters, motor bus, camera gimbals, gripper service), with a check at each step | electrical lead | **yes** |
...
| [safety → Crush](../before-you-start/safety.md#crush) | Configured per-joint torque limits, and a pinch-point diagram (knee, elbow, hip-roll/thigh, waist, gripper jaws, camera gimbals) | controls lead for the torques, hardware lead for the geometry | **yes** |
...
| Page | What is missing | Who can supply it | Blocks release |
| --- | --- | --- | :-: |
| [actuators → Which model goes in which joint](../bom/actuators.md#which-model-goes-in-which-joint) | RS05 on wrist_3 and the four camera joints comes from the code only; not checked on the robot | hardware lead + controls lead | no |
...
| [electronics → Where each part goes](../bom/electronics.md#where-each-part-goes) | Computer configuration (RAM, storage, OS, power input), camera firmware version, IMU orientation and frame, peripherals needed at bring-up | electrical lead | no |
...
| Page | What is missing | Who can supply it | Blocks release |
| --- | --- | --- | :-: |
| [cad-downloads → Whole robot](../fabrication/cad-downloads.md#whole-robot) | No manufacturing CAD published: per-part STEP, printable 3MF/STL, PDF drawings, whole-robot STEP and Fusion 360 archive | hardware lead | **yes** |
| [cad-downloads → Whole robot](../fabrication/cad-downloads.md#whole-robot) | Fusion 360 version the files come from, and STEP AP214 or AP242 | hardware lead | **yes** |
...
| [arm → Configure and label the seven actuators (step 1)](../assembly/arm.md#step-1) | Arm axis names are inferred from joint order and part names | hardware lead, from the CAD | no |
...
| [gripper → Apply the AprilTags (step 5)](../assembly/gripper.md#step-5) | AprilTag substrate, printed size, adhesive and replacement | perception + hardware lead | no |
...
| [gripper → Set the open and closed positions (step 7)](../assembly/gripper.md#step-7) | Commissioning: finding end stops safely, safe torque limit, matching both hands | controls + hardware lead | no |
| [head-and-camera-gimbal](../assembly/head-and-camera-gimbal.md) | Gimbal column procedure: fasteners, torques, locations, zeros — Per step: screws, torque, Loctite 222 use, order.; Yaw actuator location and any yaw bearing; neck keying.; Yaw and pitch zeros; hard stops. … | hardware lead | no |
| [head-and-camera-gimbal → Configure the actuators and record the camera serials (step 1)](../assembly/head-and-camera-gimbal.md#step-1) | Which physical side carries `cam_yaw_left` (ID 7) and `cam_pitch_left` (ID 8) — Computer-aided design (CAD): left at y = −65 mm;; MuJoCo model: y = +0.065 m.; A swap fails silently. | hardware lead + controls | no |
...
| [head-and-camera-gimbal → Mount the camera (step 5)](../assembly/head-and-camera-gimbal.md#step-5) | The bracket CAD names a D435 body; confirm the D436 fits before machining | hardware lead | no |
...
| [head-and-camera-gimbal → Bench-test the column (step 7)](../assembly/head-and-camera-gimbal.md#step-7) | Single-column bench harness and written test procedure | hardware lead + controls | no |
| [head-and-camera-gimbal → Mount both columns on the top plate (step 9)](../assembly/head-and-camera-gimbal.md#step-9) | Top plate central hole: round (CAD parse) or hexagonal (CAD animation) | hardware lead | no |
...
| [leg → Configure and label the six actuators (step 1)](../assembly/leg.md#step-1) | `hip_1` pitch, `hip_3` yaw: inferred from joint order | hardware lead | no |
...
| [torso-and-waist → Mount the IMU (step 8)](../assembly/torso-and-waist.md#step-8) | IMU location and orientation in the base frame, and how to verify them | hardware lead + controls | no |
...
| Page | What is missing | Who can supply it | Blocks release |
| --- | --- | --- | :-: |
| [can-bus → The actuator map](../electrical/can-bus.md#the-actuator-map) | Camera joint indices: source comments put right at 27–28, left at 29–30 (used above); the written order puts left first. Check the MJCF joint order | controls lead | no |
...
| [routing → Verify the routing](../electrical/routing.md#verify-the-routing) | What unpowered means for the wiggle test, which reads live telemetry: supplies and processes that must run are not stated | controls lead + electrical lead | no |
...
| [acceptance-tests → A5. Check zero and model fidelity](../bringup/acceptance-tests.md#a5-check-zero-and-model-fidelity) | A5 acceptable gravity-torque residual per joint | controls lead | no |
...
| [acceptance-tests → A7. Test grippers](../bringup/acceptance-tests.md#a7-test-grippers) | A7 reference no-slip levels, test object, asymmetry limit, endurance test | controls lead | no |
...
| [acceptance-tests → A9. Test arm transit](../bringup/acceptance-tests.md#a9-test-arm-transit) | A9 launch conditions: prerequisites, arm selection, torque and motor group | controls lead | no |
...
| [camera-calibration → 2. Check gimbal encoder zero](../bringup/camera-calibration.md#2-check-gimbal-encoder-zero) | Procedure and tolerance for setting the gimbal zero | perception lead + hardware lead | no |
| [camera-calibration → 3. Calibrate motor-to-camera gear](../bringup/camera-calibration.md#3-calibrate-motor-to-camera-gear) | Rear-gimbal gear calibration, designed ratio, storage, allowed spread | perception lead | no |
...
| [camera-calibration → Check intrinsics](../bringup/camera-calibration.md#check-intrinsics) | Whether D436 factory intrinsics are trusted or re-estimated | perception lead | no |
| [camera-calibration → Make the fixtures](../bringup/camera-calibration.md#make-the-fixtures) | Tag cubes: geometry, tag family and size, wrist mounts, artwork (not in the bill of materials or CAD) | perception lead | **yes** |
...
| [joint-zeroing → Set zeros](../bringup/joint-zeroing.md#set-zeros) | Zero pose: figure, per-joint table, holding method or fixture, joint order | hardware lead + controls lead | no |
...
| [motor-id-and-config → Smoke-test at 5 % torque (step 3)](../bringup/motor-id-and-config.md#step-3) | Smoke-test joints: index 13 onward may include the 4 gimbal joints | controls lead | no |
| [motor-id-and-config → Confirm directions](../bringup/motor-id-and-config.md#confirm-directions) | Positive direction and travel limits of all 31 joints: table, figure, per-joint check | controls lead | **yes** |
...
| [citation-and-license → Third-party material](citation-and-license.md#third-party-material) | Third-party terms: purchased components (SDKs, firmware, EULAs) and CAD derived from vendor models | hardware lead | **yes** |
| [faq](faq.md) | Camera module interface for another robot: mounting, power, bus | hardware lead | no |
...
| [full-specifications → Electronics, power, structure](full-specifications.md#electronics-power-structure) | Unspecified: mechanical stops, configured torque limits, mass breakdown, CoM and inertia, foot geometry, current and runtime, payload, walking speed, gimbal range and slew rate, IP rating, robot temperature range, noise | hardware lead + controls lead | no |
...
| Page | What is missing | Who can supply it | Blocks release |
| --- | --- | --- | :-: |
| [software → Robot computer setup](../software.md#robot-computer-setup) | Whether deploy has lower-body gravity compensation and stable-stand control | controls lead | no |
| [software → Robot model](../software.md#robot-model) | Asset READMEs cite paths absent from the export | controls lead | no |
| [software → Robot model](../software.md#robot-model) | Hardware/software contract: joint vector (index, name, sign), checkpoint per revision, tested OS and drivers, fresh install | controls lead | no |
