...
{{ bom_count("cables-connectors.csv") }} lines. Build it with
...
| --- | --- | ---: | ---: | ---: | --- |
{% for r in pd_read_csv("data/cables-connectors.csv", dtype="str", keep_default_na=False).to_dict("records") %}| `{{ r.part_id }}` | {{ r.description }} | {{ r.qty_per_robot }} | {{ money(r.unit_cost_usd|float) }} | {{ money((r.unit_cost_usd|float) * (r.qty_per_robot|int)) }} | [{{ r.vendor }}]({{ r.vendor_url }}) |
{% endfor %}| | | | **Total** | **{{ bom_subtotal("cables-connectors.csv") }}** | |

No bulk wire here; see [Not in this list](#not-in-this-list).
...
!!! missing "MISSING — parts-list rows for EC5 connectors, Ethernet cable, heat-shrink, bulk wire (gauge, rating, colour, length), CAN termination resistors and GH1.25 cable housings"
    *Owner: electrical lead.*
